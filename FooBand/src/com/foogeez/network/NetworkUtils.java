...
public class NetworkUtils {
    ...
    public NetworkUtils(Context context, NetworkCallback rsp) {
        mContext = context;
        mCallback = rsp;
    ...
    private static class LocalHandler extends Handler {
        private WeakReference<NetworkUtils> mContext;

        public LocalHandler(NetworkUtils context) {
            this.mContext = new WeakReference<NetworkUtils>(context);
        }

        @Override
        public void handleMessage(Message msg) {
            NetworkUtils context = mContext.get();
            if (context != null) {
                switch (msg.what) {
                case ON_PRE_EXCUTE:
    ...
    Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch(msg.what) {
                ...
                case ON_EXCUTE_SUCCESS:
                    try {
                        mCallback.OnExcuteSuccess((JSONObject)msg.obj);
                    } 
                    catch (JSONException e) {
                        e.printStackTrace();
                    }
                ...
                case ON_EXCUTE_FAILURE:
                    try {
                        mCallback.OnExcuteFailure((JSONObject)msg.obj);
                    } 
                    catch (JSONException e) {
                        e.printStackTrace();
                    }
    ...
    public void request(final int type, final String format, final Object... keyword) {
        ...
        mDoingFlag = true;
        final Runnable nRunable = new Runnable() {
            @Override
            public void run() {
                if (mDoingFlag == true) {
                    mDoingFlag = false;
        ...
        mHandler.postDelayed(nRunable, 10000);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Log.i("NetworkUtils", ((type == POST) ? "PST" : "GET") + ":" + url);
                    ...
                    mHandler.removeCallbacks(nRunable);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    /**
                     * String format = String.format(url, sid); Log.i("NetworkUtils", "GET:" + url); String result =
